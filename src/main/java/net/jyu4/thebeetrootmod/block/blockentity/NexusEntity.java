...
import net.jyu4.thebeetrootmod.registry.ModSounds;
...
public class NexusEntity extends BlockEntityBase {
    ...
    public NexusEntity(BlockPos pPos, BlockState pBlockState) {
    ...
    public static void clientTick(Level pLevel, BlockPos pPos, BlockState pState, NexusEntity pBlockEntity) {
    ...
    public static void serverTick(Level pLevel, BlockPos pPos, BlockState pState, NexusEntity pBlockEntity) {
        ++pBlockEntity.tickCount;
        long i = pLevel.getGameTime();
        List<BlockPos> list = pBlockEntity.effectBlocks;
        if (i % 40L == 0L) {
            ...
            if (flag != pBlockEntity.isActive) {
                SoundEvent soundevent = flag ? ModSounds.NEXUS_EMPOWERED.get() : SoundEvents.CONDUIT_DEACTIVATE;
                pLevel.playSound((Player)null, pPos, soundevent, SoundSource.BLOCKS, 0.10F, 1.0F);
            }
            ...
            if (flag) {
                ...
                //damageNearbyPhantoms(pLevel, pPos);
            }
        }

        if (pBlockEntity.isActive()) {
            if (i % 80L == 0L) {
                pLevel.playSound((Player)null, pPos, ModSounds.NEXUS_COMPLETION.get(), SoundSource.BLOCKS, 0.10F, 1.0F);
            }
    ...
    private static void damageNearbyPhantoms(Level pLevel, BlockPos pPos) {
        int range = 10; // Example range value, adjust as needed
        AABB aabb = new AABB(pPos).inflate(range, range, range);
        List<Phantom> phantoms = pLevel.getEntitiesOfClass(Phantom.class, aabb);

        for (Phantom phantom : phantoms) {
            phantom.hurt(pLevel.damageSources().magic(), 4.0F);
        }
    }

    private static void updateHunting(NexusEntity pBlockEntity, List<BlockPos> pPositions) {
        pBlockEntity.setHunting(true);//pPositions.size() >= 42);
    }
...
*/
        return true;//pPositions.size() >= 16;
    }
    ...
    private static void updateDestroyTarget(Level pLevel, BlockPos pPos, BlockState pState, List<BlockPos> pPositions, NexusEntity pBlockEntity) {
        ...
        if (pBlockEntity.destroyTarget != null) {
            ...
            pBlockEntity.destroyTarget.hurt(pLevel.damageSources().magic(), 5.0F);
        }
    ...
    private static void updateClientTarget(Level pLevel, BlockPos pPos, NexusEntity pBlockEntity) {
    ...
    private static LivingEntity findDestroyTarget(Level pLevel, BlockPos pPos, UUID pTargetId) {
        List<Phantom> list = pLevel.getEntitiesOfClass(Phantom.class, getDestroyRangeAABB(pPos), (p_289510_) -> {
